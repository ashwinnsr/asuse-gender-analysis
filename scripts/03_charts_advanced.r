...
data_file  <- file.path(asuse_dir, "Analysis_Ready_Data",
...
pub_theme <- theme_minimal(base_size = 15, base_family = "sans") +
  theme(
    ...
    plot.subtitle = element_text(size = 13, color = "grey40", hjust = 0,
                                  margin = margin(b = 12)),
...
p_dumbbell <- ggplot(wide_db) +
  ...
  geom_point(aes(x = Male,   y = industry), color = gender_pal["Male"],
             size = 11) +
  geom_point(aes(x = Female, y = industry), color = gender_pal["Female"],
             size = 11) +
  ...
  geom_text(aes(x = Female, y = industry, label = paste0(round(Female, 1), "%")),
            color = "white", size = 4.2, fontface = "bold") +
  annotate("text", x = wide_db$Male[1] + 1,   y = 2.35, label = "Men",
           color = gender_pal["Male"],   fontface = "bold", size = 4) +
  annotate("text", x = wide_db$Female[1] - 1, y = 2.35, label = "Women",
  ...
  labs(
    ...
    caption  = paste0(
      ...
      sprintf("Apparel: Women %.1f%% vs Men %.1f%%  |  ",
              wide_db$Female[2], wide_db$Male[2]),
      sprintf("Tobacco: Women %.1f%% vs Men %.1f%%",
...
p_ridge <- ggplot(ridge_df, aes(x = Income / 1000, y = Group,
...
p_violin <- ggplot(violin_df, aes(x = Gender, y = Assets_K,
                                   fill = Gender, weight = Weight)) +
...
png(file.path(out_dir, "E_correlogram_financial_structural.png"),
    width = 2400, height = 2000, res = 200, bg = "white")
corrplot::corrplot(
  cor_mat,
  method   = "ellipse",
  type     = "lower",
  ...
  number.cex  = 0.75,
  cl.cex      = 0.85,
  title       = "Pairwise Correlations — ASUSE 2023-24 Enterprise Variables",
  mar         = c(0, 0, 2, 0)
)
dev.off()
...
make_mini_bar <- function(data, col, title, ylab = "Percent (%)") {
  d <- data |> select(Gender, value = all_of(col)) |>
